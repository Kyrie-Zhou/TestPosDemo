 ...
 * 输入pin的activity界面
 */
public class PINActivity extends AppCompatActivity implements PINBroadCastRev.ChannelRecv {
    ProgressDialog progressDialog;//进度条圆圈
    boolean isCancel = false;//是否按下ped的取消键
    TextView txv_pin;
    Transaction transaction;
    OkHttpClient client ;
    MyDatabaseHelper myDatabaseHelper;
    PINBroadCastRev pinBroadCastRev;//注册的广播
    final static int NETWORK_TIMEOUT = 0;//网络连接超时 默认设置5秒
    final static int CONFIRM_PINBLOCK = 1;//ped输入确认完毕
    final static int PRINT_EXCEPTION = 2;//打印异常
    final static int PIN_NOTINPUT = 3;//ped长时间未输入
    final static int PIN_CANCEL = 4;//ped按键取消
    Handler handler ;
    static class MyHandler extends Handler {
        WeakReference<PINActivity> mWeakReference;
        public MyHandler(PINActivity activity) {
            mWeakReference = new WeakReference<PINActivity>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            PINActivity activity = mWeakReference.get();
            if (activity != null) {
                switch (msg.what){
                    case NETWORK_TIMEOUT://网络超时
                        activity.progressDialog.dismiss();
                        activity.showMyDialog("出错","网络连接超时，请稍后重试...")
                                .setCancelable(false)
                    ...
                    case CONFIRM_PINBLOCK://ped输入确认完毕
                    ...
                    case PRINT_EXCEPTION://打印异常
                        activity.showMyDialog("出错","打印机发生错误，请检查后后重试...").show();
                        break;

                    case PIN_NOTINPUT://ped长时间未输入
                        activity.showMyDialog("警告","长时间未输入，暂时取消交易，请重试！")
                                .setCancelable(false)
                                .setPositiveButton("OK",(dialog, which) -> {
                                    activity.finish();
                                }).show();
                    ...
                    case PIN_CANCEL://ped按键取消
                        activity.showMyDialog("警告","交易已取消！")
                                .setCancelable(false)
                                .setPositiveButton("OK",(dialog, which) -> {
                                    activity.finish();
                                }).show();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                e.printStackTrace();//当点击按键取消时会报异常，当长时间未输入超过指定时间会报异常
                Message message = new Message();
                message.what = PIN_NOTINPUT;
                if(isCancel == true){
...
//                    intent.putExtra("bitmap",bitmap);
    ...
    public byte[] compressBitmapToByteArray(Bitmap bitmap){
        ByteArrayOutputStream stream = new ByteArrayOutputStream();//创建输出流
        bitmap.compress(Bitmap.CompressFormat.PNG, 100, stream);//将bitmap进行压缩
    ...
    protected void onDestroy() {
        super.onDestroy();
        unregisterReceiver(pinBroadCastRev);//因为是动态注册广播，所以必须取消注册广播
    }
